...
public class LeakyBucketRateLimiter implements RateLimiterAlgorithm {
    ...
    public RateLimitResult isAllowed(RateLimitRule rateLimitRule) {
        ...
        String script = LuaScriptModel.getLeakBucketRateLimiterScript();
        List<Long> results = rScript.eval(RScript.Mode.READ_WRITE, script, RScript.ReturnType.MULTI, keys,
                rateLimitRule.getBucketCapacity(),
                rateLimitRule.getRate(),
                rateLimitRule.getLeakyBucketRate());
